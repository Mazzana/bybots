...
export async function relayGateway(label: string) {
  const state = { roster: [] as unknown[], outbox: [] as unknown[], replies: [] as Record<string, unknown>[], deliveries: [] as Record<string, unknown>[] };
  ...
  sockets.on("connection", (socket, request) => {
    ...
    socket.on("message", (raw) => {
      ...
      if (method === "bot_relay.deliver") { state.deliveries.push(params); result = { reply: `${label} reviewed the request.` }; }
      if (method === "bot_relay.reply") { state.replies.push(params); result = { ok: true }; }
