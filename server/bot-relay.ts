...
export class BotRelay {
  ...
  journalError = false;
  rateLimited = false;
  get journalFull() { return this.records.length >= 4096; }
  constructor(private readonly connections: () => RelayConnection[], private readonly journal?: RelayJournalStore) {}

  async initialize() {
    try {
      const records = await this.journal?.load() || [];
      for (const record of records) {
        if (record.status === "delivering" || record.status === "reply-pending") record.status = "uncertain";
        this.seen.add(`${record.source}:${record.id}`);
      }
      this.records.push(...records);
      this.activity.push(...records.slice(-50).reverse());
      await this.persist();
    } catch { this.journalError = true; }
  }
  private persist() {
    const operation = this.writes.then(async () => {
      if (this.journalError) throw new Error("Relay journal unavailable");
      await this.journal?.save(this.records.map((row) => ({ ...row })));
    });
    this.writes = operation.catch(() => { this.journalError = true; });
    return operation;
  }
  ...
  async tick(forceSync = false) {
    if (this.closed || this.busy || this.journalError) return;
    this.busy = true;
    try {
      const connections = this.connections();
      this.rateLimited = this.records.filter((row) => row.at > Date.now() - 10 * 60_000).length >= 30;
      ...
      if (forceSync || Date.now() - this.lastSync > 60_000) {
        ...
        await Promise.all(connections.map(async (row) => {
          ...
          } catch { this.rosters.delete(row.id); this.status.set(row.id, "unavailable"); }
        }));
        await Promise.all(connections.map(async (row) => {
          try {
            if (!this.current(row)) return;
            ...
            if (this.rosters.has(row.id)) this.status.set(row.id, "ready");
      ...
      await Promise.all([...this.pending].map(async ([key, pending]) => {
        if (Date.now() - pending.at > 10 * 60_000 || !this.current(pending.sender)) {
          this.pending.delete(key); pending.activity.status = "uncertain"; await this.persist(); return;
        }
        await this.postReply(key, pending);
      }));
      await Promise.all(connections.map(async (sender) => {
        if (!this.current(sender) || this.deliveries.size >= 16 || this.pending.size >= 128 || this.rateLimited || this.records.length >= 4096) return;
        try {
          ...
          for (const raw of response.envelopes) {
            ...
            if (this.seen.has(key) || this.records.length >= 4096) continue;
            this.seen.add(key);
            const delivery = this.deliver(sender, envelope, key).catch(() => { this.journalError = true; }).finally(() => this.deliveries.delete(delivery));
    ...
    } catch { this.journalError = true; } finally { this.busy = false; }
  }
  ...
  private async deliver(sender: RelayConnection, envelope: z.infer<typeof envelopeSchema>, key: string) {
    const withinRate = this.records.filter((row) => row.at > Date.now() - 10 * 60_000).length < 30;
    const withinConcurrency = this.deliveries.size < 16;
    ...
    // Persist intent before any side effect. An interrupted turn is never retried.
    await this.persist();
  ...
  private async postReply(key: string, pending: { sender: RelayConnection; payload: Record<string, unknown>; activity: RelayActivity }) {
    ...
    try {
      ...
      await this.persist();
    } finally { this.replying.delete(key); }
  }

  async settle() { await Promise.all([...this.deliveries]); await this.writes; }
