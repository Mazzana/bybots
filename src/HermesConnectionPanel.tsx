...
export function HermesConnectionPanel({ api, role, initialLocalUnavailable = false, autoReconnect = false, additional = false, onConnected }: HermesConnectionPanelProps) {
  ...
  return <div className="gateway-settings">
    ...
    {target === "local" ? showLocalUnavailable ? <div className="local-gateway-error" role="alert">
      ...
      <FormField label={t("Gateway URL")} help={additional ? t("To use another address, add a gateway. This connection stays separate.") : undefined}><input type="url" inputMode="url" value={baseUrl} readOnly={additional} disabled={!canManage || Boolean(busy)} onChange={(event) => { setBaseUrl(event.target.value); setAuthProbe(null); setSuccess(""); }} placeholder="https://hermes.example.com" autoComplete="url" required /></FormField>
